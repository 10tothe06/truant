...
/// <summary>
/// Entirely different approach: Meandering Spine + Variable Width.
/// 
/// Instead of starting from a closed blob and perturbing it, we first build a
/// short meandering centerline (the "spine"), then extrude left/right with a
/// width that varies strongly along the length. This naturally produces
/// elongated, irregular, often kidney- or ribbon-shaped lakes that are
/// far from circular.
/// 
/// Vertices are returned in clockwise order.
/// </summary>
public static class LakeVertexGenerator2
{
    /// <param name="vertexCount">Desired number of final shoreline vertices</param>
    /// <param name="length">Approximate length of the lake along its long axis</param>
    /// <param name="maxWidth">Maximum width (perpendicular to the spine)</param>
    /// <param name="meander">How much the spine bends (0.2 = gentle, 1.0 = very twisty)</param>
    /// <param name="widthVariation">How irregular the width is along the length</param>
